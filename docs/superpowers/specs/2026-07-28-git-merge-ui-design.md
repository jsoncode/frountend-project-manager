...
- 合并代码时提供接近 WebStorm 的体验：先弹框列出待处理文件（冲突标红），再支持整文件采用本人/他人，或打开三栏 Diff 逐块合并。
- 可随时**取消合并**（`git merge --abort`），撤销本次合并。
- 全部冲突解决后，用户确认再 **完成合并**（`git commit`）。
- 打开/刷新项目时检测未完成合并；当前分支旁显示「合并中」；右键可「继续合并」进入同一套 UI。
- 覆盖入口：**右键合并到当前分支**、**拉取产生冲突**、**继续未完成合并**。
